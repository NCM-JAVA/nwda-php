 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
 			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM manage_vacancy WHERE id=$deleteid";
					$sql="Delete From manage_vacancy where id='$deleteid'";
					$res=mysql_query($sql);
...
if(($inactiveid !=''))
{
   ...
   $sql="Update manage_vacancy set status='1' where m_id='$inactiveid'";
 $res=mysql_query($sql);
...
<?php 
	if($btnsubmit=="Search")
	{
		if($filter_search!='')
		{
				...
				$errmsg = 'Name must be from letters that should be minimum 2 and maximum 20.<br>';
				 ...
				 {
					$querywhere .=" and pro_title LIKE '%$filter_search%'"; 
				 }
		}
		if($texcat!='')
		{
  ...
  $querywhere .=" and status=$txtstatus"; 
	...
	if($_GET['txtstatus'] !='')
	 {
			...
			else { $where=" and status =". $_GET['txtstatus'];} 
	 }
	 else { $where= "and status=3"; }
...
$wherecluse=" and language_id=$language and status='$txtstatus'";
}
...
<a href="vacancy.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
<?php if($role_map['medit']=='ED' || $user_id=='101'){?><li id="editer" class="edit-icon"><a href="#" onclick="alert('Please Select Manage Vacancy ')" title="Edit">Edit</a><?php }?></li>
             <?php if($role_map['mdelete']=='DE' || $user_id=='101'){?> 
			 <?php if ($txtstatus!='3'){?>
               <li id="delete" class="delete-icon"><a href="#" onclick="alert('Please Select Manage Vacancy')" title="Delete">Delete</a></li><?php } } ?>   
            </ul>
...
{
	?>
<option value="<?php echo $value['c_id']; ?>" <?php if($value['c_id']==$texcat){ echo 'selected="selected"'; } else { }?>><?php echo $value['c_name']; ?></option>
<?php }
 ?>
</select>
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
          ...
          <div id="list">
				<div id="response"> </div>
							...
							<span class="space-menuname_m" style="width:22%;"><input type="radio"  name="radio1" id="<?php echo $id; ?>"  value="<?php echo $id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $job_title; ?></span>
							<span class="space-lang" style="width:22%;"><?php echo $location['state_name']; ?></span>
							<span class="space-lang" style="width:14%;"><?php echo $category['c_name']; ?></span>
							<span class="space-option" style="width:13%;"><?php language($language_id); ?></span>
							<span class="space-lang" style="width:13%;"><?php status($status); ?></span>
							<span class="space-option" style="width:15%;">
                            <?php  if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_vacancy.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('vacancyview.php?page_id=<?php echo $id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
