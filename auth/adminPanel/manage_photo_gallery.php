...
<?php    
	if($btnsubmit=="Search"  || $_GET['page_query'] =="Search"){
		...
		if($errmsg==''){
			$query ="select * from photogallery  where gallery_type='1' $querywhere  ORDER BY page_position DESC";
		}else{
		...
		$query ="select * from photogallery $wherecluse  ORDER BY page_position DESC";
	}
...
<select name="txtstatus" id="txtstatus"  class="inputbox" autocomplete="off">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
						...
						<?php	 
							$pager = new PS_Pagination($link, $result,"txtstatus=$txtstatus&page_query=Search");
							$rs = $pager->paginate();
							$result->num_rows;
						...
						<p><?php echo $pager->renderFullNav();?></p>
								<?php }
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="photogallery.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
