...
if(isset($_POST['Submit_g']) && $_GET['id']=='')
		...
		if(trim($cat_id)=="")
				...
				else if(preg_match("/^[a-zA-Z0-9 _.,:\"\']{3,50}+$/", $txtename) === 0)
				{
				$errmsg .= "name must be Alphabet and Special Characters( _.,: ) that should be minimum 3 and maximum 50."."<br>";
				}
				if(trim($txtenamehi)=="")
				{
			...
			if($errmsg=='')
			{
			$tableName_send="whatsnew_category";
			$tableFieldsName_old=array("c_name","c_namehi","c_status","c_type");
			$tableFieldsValues_send=array("$txtename","$txtenamehi","$user_status","$cat_id");
					...
					$msg=CONTENTADD;
					$_SESSION['SESS_MSG']=$msg;
					header("location:manage_new_category.php");
					exit;	
...
if(isset($_POST['Submit_g']) && $_GET['id']!='')
	...
	$old =array("c_name","c_namehi","c_status","c_type");
 ...
 if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("delete from whatsnew_category where c_id='$did'");
...
<a href="gallery-category.php" title="New"><span class="icon-28-new"></span>New</a></li>
	...
	if($_GET['id']!='')
	{
		$rq = mysql_query("select * from whatsnew_category where c_id='".$_GET['id']."'");
...
<div class="frm_row"> 
			<span class="label1">
			<label for="hometype">Category Type:</label>
			<span class="star">*</span></span> <span class="input1">
			<select name="hometype" id="hometype"  autocomplete="off">
			<option value=""> Select </option> 	
			<option value="1" <?php if($rr['c_type'] == 1){ echo "selected='selected'";}?> >What's New</option>
			<option value="2" <?php if($rr['c_type'] == 2){ echo "selected='selected'";}?> >Circular</option>
			</select>
			</span>
			<div class="clear"></div>
			</div>
      ...
      <div class="frm_row"> <span class="button_row">
		<input name="Submit_g" type="submit" class="button" id="cmdsubmit" value="Submit" />
            ...
            <input name="cmdreset" type="reset" class="button" id="cmdreset" value="Reset" />
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'manage_new_category.php';" />
	</span>
</div>
<div class="clear"></div>
...
<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc">
	...
	<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Hindi Name</span></th>
	 <th width="63" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Category Type</span></th>
	...
	<?php 
	$columns = "select * ";
$sql = "from whatsnew_category where 1 ";
  ...
  <tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
    <td width="38" align="left"  class="left-tdtext"><?php echo ++$counter;?></td>
      <td width="510" align="left" class="left-tdtext"><?php echo html_entity_decode($data['c_name']);?></td>
		...
		<td width="510" align="left" class="left-tdtext"><?php if($data['c_type'] == 1){ echo "What's New";}else{ echo "Circular";};?></td>
	   <td width="50" align="left" class="left-tdtext"><?php echo html_entity_decode($status);?></td>
    <td width="47" align="center" class="left-tdtext"><a href="manage_new_category.php?id=<?php echo $data['c_id'];?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>
	
    <td width="63" align="center" class="left-tdtext"><a href="manage_new_category.php?did=<?php echo $data['c_id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
  </tr>
<?php	}?>
	<tr>
