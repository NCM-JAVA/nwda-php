...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM combine WHERE m_id=$cid";
   ...
   unlink($image_path2);	
					...
					if($res)
					{	
					header("location:delete.php?status=minister_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update combine set approve_status='1' where m_id='$inactiveid'";
 $res=mysql_query($sql);
  $sql="Update combine_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysql_query($sql);
	if($res)
	{	
	header("location:delete.php?status=minister_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage  Minister Speech: DDA</title>
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
 $query ="select * from combine where 1 and cat_id =7 $querywhere ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
$query ="select * from combine  where 1 and cat_id =7 $wherecluse ORDER BY page_postion ASC ";
}
?>      
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Minister Speech</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="speeach.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="minister_speech.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="speeach.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="minister_speech.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
