 ...
 if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("delete from category where c_id='$did'");
...
<title>Manage Media Categories : DDA</title>
...
<div class="closestatus" style="float: none;">
<p class="closestatus" style="float: right;"><img alt="Attention" src="images/close.png" class="margintop"></p>
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['SESS_MSG']; ?></p>
</div>
</div>
          <?php }?>	
      	<div class="clear"></div>
...
<a href="gallery-category.php" title="New"><span class="icon-28-new"></span>New</a></li>
	...
	if($_GET['id']!='')
	{
		$rq = mysql_query("select * from category where c_id='".$_GET['id']."'");
