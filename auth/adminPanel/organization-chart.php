...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
	{
		...
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
			...
			if($check_status >0)
			{
					$sql="Delete From  organizationchart where id='$deleteid'";
					$res = $conn->query($sql);
					
					$SQL1 = "SELECT * FROM audit_trail where page_id='$deleteid'";
				    $Query = $conn->query($SQL1);
					
					// $txtename  = mysql_result($Query,0,'page_name');
				...
				/*	$sql5 = "INSERT INTO `audit_trail`(`user_login_id`, `page_id`, `page_name`, `page_action`, `page_category`, `page_action_date`, `ip_address`, `lang`, `page_title`, `approve_status`)VALUES('$user_id','$page_id','$txtename','$action','$module_id','$date','$ip','$txtlanguage','$gallery_categoryname','$txtstatus')";
					$sql21 = $conn->query($sql5);*/


					if($res)
					{	
					header("location:delete.php?status=orgdeleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update  organizationchart set approve_status='1' where id='$inactiveid'";
 $res=$conn->query($sql);
  
	if($res)
	{	
	header("location:delete.php?status=orginactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Organization Chart: National Water Development Agency Administration</title>
...
<?php 
	...
	if($btneng!='')
	  ...
	  if($designation!='')
	  {
	  $querywhere .=" and designation=$designation";
	  }
	  if($errmsg=='')	
	  {
	  	if($user_id!='101') {
	  $query ="select * from organizationchart where $querywhere and designation='$aid' ORDER BY page_postion ASC "; 
	 } else {
	  $query ="select * from organizationchart where $querywhere ORDER BY page_postion ASC "; 
	  }
	  }
	 else
	{
...
if($role_map['draft']=='DR' || $role_map['mapprove']=='AP'  || $user_id=='101' || $user_id=='104'){
	if($_GET['txtstatus'] !='')
	 ...
	 else { if($user_id=='104'){ $cat="and category_id=2"; } else { $cat=''; } $where= "and approve_status=1  $cat"; }
...
if($user_id!='101') {
	  $query ="select * from organizationchart where 1 $wherecluse  and designation='$aid' ORDER BY page_postion ASC"; 
	 } else {
	  $query ="select * from organizationchart wherecluse ORDER BY page_postion ASC"; 
	  }
}
?>      
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Organization Chart </h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?>
	<li class="new-icon"><a href="add_organizationchart.php" title="New"><span class="icon-28-new"></span>New</a></li>
<?php }?>
<?php if($role_map['medit']=='ED' || $user_id=='101' || $aid!='' && $aid!='0'){?>
	<li id="editer" class="edit-icon"><a href="#" onclick="alert('Please select atleast one radio button!')" title="Edit">Edit</a></li>
<?php }?>
<?php if($role_map['mdelete']=='DE' || $user_id=='101'){?> 
	<?php if ($txtstatus!='3'){?>
		<li id="delete" class="delete-icon"><a href="#" onclick="alert('Please select atleast one radio button!')" title="Delete">Delete</a></li>
<?php 	} 
	} ?>   
            </ul>
			...
			<?php if($role_map['pending']=='PND'){?>
					...
					<?php }if($role_map['mapprove']=='AP' || $user_id=='101' || $user_id=='104' ){?>
                      <option value="2" <?php if ($txtstatus=='2') echo 'selected="selected"';?>>Approval</option>
										<?php }
					if($role_map['publish']=='PB' || $user_id=='101' || $user_id=='104' || $aid != ''){?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response2"> </div>
							...
							<span class="space-menuname_m"><input type="radio"  name="radio1" id="<?php echo $id; ?>"  value="<?php echo $id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $name; ?></span>
							<?php if($data['designation']!='') { ?>
							<span class="space-pagetitle">
							<?php 
								$sql ="select * from org_setup where deg_id=".$data['designation']."";
								$rs = $conn->query($sql);
								$row = $rs->fetch_array();
							...
							<?php } else { ?>
							<span class="space-pagetitle">* Vacant </span>
							<?php } ?>
							<span class="space-lang"><?php status($approve_status); ?></span>
							<span class="space-option"><?php language($language_id); ?></span>

							<span class="space-option">
                            <?php  if($role_map['publish']=='PB' || $txtstatus=='3' && $aid==""){?>
							<a href="organization-chart.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Do you want to inactive this record?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('organizationview.php?page_id=<?php echo $id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=update_org'; 
		...
		jQuery("#list2 ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=update_org'; 
			jQuery.post("updateList.php", order, function(theResponse){
				jQuery("#response2").html(theResponse);
				jQuery("#response2").slideDown('slow');
				slideout();
			}); 															 
		}								  
		});
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="edit_organizationchart.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="organization-chart.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Do you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
			  //add the content retrieved from ajax and put it in the #content div
