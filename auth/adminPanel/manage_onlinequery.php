...
if($check_status >0)
{
		$sql="Delete From onlinequery_form where id='$deleteid'";
		$res=mysql_query($sql);
		...
		if($res)
		{	
		header("location:delete.php?status=deleteonlinequery");
		}
}
else
{
...
/*if(($inactiveid !=''))
{
   ...
   $sql="Update onlinequery_form set approve_status='1' where id='$inactiveid'";
 $res=mysql_query($sql);
...
<head>
<title>Manage Online Query : <?php echo $sitename; ?></title>
...
function editlist(id) {
	...
	$('#loading').hide(); 
		...
		{
            //hide the progress bar
			var e='<a href="edit_onlinequery.php?editid='+dataid+'" title="Reply"><span class="icon-28-edit"></span>Reply</a>';
			var d='<a href="manage_onlinequery.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
			  //$('#loading').hide();   
				...
				if($btnsubmit=="Search")
				{
					if($btneng!='')
					{
						...
						$_SESSION['lname']=$btneng;
							...
							//$querywhere .=" and language_id=$language";//language_id colum is not impleted in table
					}
					if($errmsg=='')	
					{
						//echo $query ="select id,name,email,contact_no,review_comment,review_date from onlinequery_form_temp  where create_date >= CURRENT_DATE() - INTERVAL 3 MONTH $querywhere $datef ORDER BY id DESC";

						$query ="select id,name,email,phone,review_comment,review_date from onlinequery_form  where review_comment IS NULL $querywhere $datef ORDER BY id DESC";
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Online Query
</h3>
 ...
 <?php if($_SESSION['admin_auto_id_sess']=='101'){ ?>
	<!-- <li>
		<a href="download_onlinequery_excel.php" title="Export Online Query"><span class="icon-28-new"></span>Export Online Query</a>
	</li> -->
	
	<li>
		<a href="archive_onlinequery.php" title="Archive"><span class="icon-28-new"></span>Archive</a>
	</li>
...
<div class="grid_view">
<div class="new-gried">
	<form id="manage_onlinequery" name="manage_onlinequery" method="post" action="">
  ...
  <div id="list">
    <div id="response"> </div>  
		...
		<span class="space-option">
			<a href="" class="cat_link" title="View" onclick="MM_openBrWindow('onlinequeryview.php?page_id=<?php echo $id;?>','window','width=900,height=400,scrollbars=yes')">View </a> 
		</span>
		<div class="clear"></div>
		</li><?php }?>
...
<!-- Container div-->
  ...
  <script type="text/javascript">
		  function getUrl(id) {
			  ...
			  document.getElementById('edit').href='edit_onlinequery.php?editid='+id+'<?php echo $pagnn; echo '&'.$string_url;?>';
		  }
	  </script>
