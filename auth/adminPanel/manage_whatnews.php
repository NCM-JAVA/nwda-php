...
// $role_id_page=role_permission_page($user_id,$role_id,$model_id);
	...
	if($deleteid !='')
	{
		...
		if($check_status >0)
		{
			...
			if($res){	
				header("location:delete.php?status=whatnews_deleteid");
			}
		}
		else{
			$msg = "Login to Access Admin Panel";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage What's New: <?php echo $sitename;?></title>
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
 $query ="select * from combine where cat_id=1 $querywhere ORDER BY m_id DESC "; 
	  }
	 else
	{
...
$wherecluse=" and language_id=$language and approve_status='$txtstatus'";
 ...
 $query ="select * from combine where cat_id=1 $wherecluse ORDER BY m_id DESC ";
}
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage What's New</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="whatsnews.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_whatnews.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="whatsnews.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_whatnews.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
