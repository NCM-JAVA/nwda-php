...
if(isset($_POST['Submit_g']) && $_GET['id']=='')
{
    ...
    if(trim($c_name)=="")
	{
		$errmsg .="Please enter module name."."<br>";
	}
	...
	if(trim($user_status)=="Select")
	{
		...
		if($errmsg=='')
		{
			$tableName_send="module_master";
			$tableFieldsName_old=array("c_name","c_name_hi","status");
			$tableFieldsValues_send=array("$c_name","$c_name_hi","$user_status");
			$value=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);
			//$page_id=mysql_insert_id();
			$msg=CONTENTADD;
			$_SESSION['SESS_MSG']=$msg;
			header("location:module_master.php");
			exit;
}
...
$errmsg="";
                ...
                if(trim($c_name)=="")
	{
		$errmsg .="Please enter State name."."<br>";
	}
	...
	if($errmsg=='')
	{
	   ...
	   $old =array("c_name","c_name_hi","status");
		$new =array("$c_name","$c_name_hi","$user_status");
...
if($_GET['did']!='')
{
	if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("DELETE FROM `module_master`where c_id='$did'");
...
<head>
	...
	<title>Manage Module Master : <?=$sitename; ?></title>
		...
		<div class="right_col1">
			...
			<div class="addmenu">
				<div class="internalpage_heading">
					<h3 class="manageuser">Manage Module Master </h3>
				...
				<div class="grid_view">
					<form action="" method="post" enctype="multipart/form-data" style="margin:0px; padding:0px;">
						...
						if($_GET['id']!='')
						{
						     $rq = mysql_query("select * from  module_master where c_id='".$_GET['id']."'");
...
?>
              ...
              <label for="categoryename"> Module Name:</label>
              <span class="star">*</span></span> <span class="input1">
              <input name="c_name" type="text" size="50" class="input_class" id="" value="<?php echo html_entity_decode($rr['c_name']);?>" />
</span>
							<div class="clear"></div>
						</div>
						...
						<div class="frm_row"> <span class="label1">
              <label for="statenamehi">Module Hindi Name:</label>
            </span> <span class="input1">
			   <input name="c_name_hi" type="text" size="50" class="input_class" id="" value="<?php echo html_entity_decode($rr['c_name_hi']);?>" />
	...
	<select name="user_status" id="user_status" autocomplete="off">
		...
		<input name="Submit_g" type="submit" class="button" id="cmdsubmit" value="Submit" />
            ...
            <input name="cmdreset" type="reset" class="button" id="cmdreset" value="Reset" />
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'State_Master.php';" />
	</span>
						</div>
						<div class="clear"></div>
					...
					<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc">
						<tr bgcolor="whitesmoke">
							...
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Module Name</span></th>
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Module Name (Hindi)</span></th>
						...
						<?php
						$sql="SELECT * FROM `module_master` order by `c_name` ";
						...
						<?php	}else	{	?>
							...
							while($data=mysql_fetch_array($rows)){
								...
								<tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
									...
									<td width="510" align="left" class="left-tdtext"><?php echo html_entity_decode($data['c_name_hi']);?></td>
									<td width="50" align="left" class="left-tdtext"><?php echo html_entity_decode($status);?></td>
									<td width="47" align="center" class="left-tdtext"><a href="module_master.php?id=<?php echo $data['c_id'];?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>

									<td width="63" align="center" class="left-tdtext"><a href="module_master.php?did=<?php echo $data['c_id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
								</tr>
							<?php	}?>
							<tr>
