...
if($deleteid !='')
{
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql5="Delete From faq where f_id='$deleteid'";
					...
					if($res)
					{	
					header("location:delete.php?status=faq_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update faq set approve_status='1' where f_id='$inactiveid'";
 $res=mysql_query($sql);
  if($res)
	{	
	header("location:delete.php?status=faq_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage FAQ :: <?php echo $sitename;?></title>
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
 $query ="select * from faq where 1  $querywhere "; 
	  }
	 else
	{
...
$wherecluse=" and language_id=$language and approve_status='$txtstatus'";
 ...
 $query ="select * from faq where 1 $wherecluse  ";
}
?>      
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage FAQ</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="faq.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<span class="space-menuname_pg"><input type="radio"  name="radio1" id="<?php echo $f_id; ?>"  value="<?php echo $f_id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $title; ?></span>
							...
							<a href="manage_faq.php?inactiveid=<?php echo $f_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('managefaqview.php?page_id=<?php echo $f_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							<?php if($role_map['medit']=='ED' || $user_id=='101'){?> / <a href="faq.php?editid=<?php echo $f_id; ?>" title="Edit" title="View"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a> / <?php  }  ?>
							<?php if($role_map['mdelete']=='DE' || $user_id=='101'){?><a href="manage_faq.php?deleteid=<?php echo $f_id;?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a><?php  }  ?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="faq.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_faq.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
