...
if(isset($_POST['Submit_g']) && $_GET['id']=='')
{
	...
	if(trim($user_status)=="Select")
	{
		...
		if($errmsg=='')
		{
			$tableName_send="state";
			$tableFieldsName_old=array("state_name","state_hi_name","status");
			$tableFieldsValues_send=array("$statename","$statenamehi","$user_status");
...
$errmsg="";
	...
	if($errmsg=='')
	{
		...
		$old =array("state_name","state_hi_name","status");
		$new =array("$statename","$statenamehi","$user_status");
...
if($_GET['did']!='')
{
	if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("delete from state where state_id='$did'");
...
<head>
	...
	<title>Manage State Master : <?=$sitename; ?></title>
		...
		<div class="right_col1">
			...
			<div class="addmenu">
				<div class="internalpage_heading">
					<h3 class="manageuser">Manage State </h3>
				...
				<div class="grid_view">
					<form action="" method="post" enctype="multipart/form-data" style="margin:0px; padding:0px;">
						...
						if($_GET['id']!='')
						{
						     $rq = mysql_query("select * from state where state_id='".$_GET['id']."'");
...
?>
              ...
              <input name="statename" type="text" size="50" class="input_class" id="" value="<?php echo html_entity_decode($rr['state_name']);?>" />
</span>
							<div class="clear"></div>
						</div>
						...
						<div class="frm_row"> <span class="label1">
              <label for="statenamehi">State Hindi Name:</label>
              <span class="star">*</span></span> <span class="input1">
			   <input name="statenamehi" type="text" size="50" class="input_class" id="" value="<?php echo html_entity_decode($rr['state_hi_name']);?>" />
             ...
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'State_Master.php';" />
					...
					<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc">
						<tr bgcolor="whitesmoke">
							...
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">State Name</span></th>
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">State Hindi Name</span></th>
						...
						<?php	}else	{	?>
							...
							while($data=mysql_fetch_array($rows)){
								...
								<tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
									...
									<td width="510" align="left" class="left-tdtext"><?php echo html_entity_decode($data['state_name']);?></td>
									<td width="510" align="left" class="left-tdtext"><?php echo html_entity_decode($data['state_hi_name']);?></td>
									<td width="50" align="left" class="left-tdtext"><?php echo html_entity_decode($status);?></td>
									<td width="47" align="center" class="left-tdtext"><a href="State_Master.php?id=<?php echo $data['state_id'];?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>

									<td width="63" align="center" class="left-tdtext"><a href="State_Master.php?did=<?php echo $data['state_id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
								</tr>
							<?php	}?>
							<tr>
