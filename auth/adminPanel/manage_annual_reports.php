...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM annual_report WHERE m_id=$cid";
					$sql="Delete From annual_report where m_id='$deleteid'";
					$res=mysql_query($sql);
   ...
   unlink($image_path2);	
					...
					if($res)
					{	
					header("location:delete.php?status=reports_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update annual_report set approve_status='1' where m_id='$inactiveid'";
 $res=mysql_query($sql);
  $sql="Update annual_report_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysql_query($sql);
	if($res)
	{	
	header("location:delete.php?status=reports_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Annual Reports: DDA</title>
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
 $query ="select * from annual_report where 1 $querywhere ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
$query ="select * from annual_report where 1 $wherecluse ORDER BY page_postion ASC ";
}
?>      
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Annual Reports</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="annual_reports.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_annual_reports.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('reportview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=annual_report'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="annual_reports.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_annual_reports.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
