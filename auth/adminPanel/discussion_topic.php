...
$msg = "Login to Access Admin Panel";
	...
	if(isset($_POST['Submit_g']) && $_GET['id']==''){
		...
		if($_SESSION['logtoken']!=$_POST['random']){
			...
			if($errmsg ==""){
				...
				$sql="insert into threadlist set Title='$title',short_des='$txtename',CreationDate='$doj',Author='$user_name',Posts ='1',t_status='$user_status'";
				$res=$conn->query($sql);
					
				$id=$conn->insert_id;
				$sql1 = "insert into messagelist set Message='$txtename',CreationDate='$doj',m_status='$user_status',flag_id='0',ThreadID='$id',Author='$user_name'";
	...
	if(isset($_POST['Submit_g']) && $_GET['id']!=''){
		if($_SESSION['logtoken']!=$_POST['random']){
			...
			if($errmsg ==""){
				$doj=date("Y-m-d h:m:s");
			 	$sql = "UPDATE `threadlist` SET `Title`='$title',`short_des`='$txtename',`CreationDate`='$doj',`Author`='$user_name',`t_status`='$user_status' WHERE `ThreadID` ='$id'";	
	...
	if($_GET['did']!='')
	{
		$sql1 = "delete from messagelist where ThreadID='$did'";
		$rs1 = $conn->query($sql1);
		
		$sql2 = "delete from threadlist where ThreadID='$did'";
		$rs2 = $conn->query($sql2);
...
<title>Manage Discussion Topic : <?php echo $sitename;?></title>
...
<p><img alt="Attention" src="images/approve.png" class="margintop"> <span>Attention! </span><?php echo $_SESSION['SESS_MSG']; ?></p>
 ...
 <h3 class="manageuser">Manage Discussion Topic </h3>
...
<a href="gallery-category.php" title="New"><span class="icon-28-new"></span>New</a></li>
	...
	<?php	
		if($_GET['id']!='')
		{
			 $rq = "select * from threadlist where ThreadID='".$_GET['id']."'";
			$rs = $conn->query($rq);
			$rr = $rs->fetch_array();
		}
	?>   
...
<div class="frm_row"> 
			...
			<div class="clear"></div>
              ...
              <span class="star">*</span></span> <span class="input1">
			 <div id="chars"><span class='date'>Maximum 250 characters</span></div> <textarea id="text" name="txtename" maxlength="250" cols="40" rows="7" placeholder="Enter Message"><?php echo html_entity_decode($rr['short_des']);?></textarea> 	
      ...
      <div class="frm_row"> <span class="button_row">
		<input name="Submit_g" type="submit" class="button" id="cmdsubmit" value="Submit" />
            ...
            <input name="cmdreset" type="reset" class="button" id="cmdreset" value="Reset" />
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'discussion_topic.php';" />
	</span>
</div>
<div class="clear"></div>
...
?>
  <tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
    ...
    <td width="47" align="center" class="left-tdtext"><a href="discussion_topic.php?id=<?php echo $data['ThreadID'];?>" class="bluelink" title="Edit">Edit</a></td>
    <td width="63" align="center" class="left-tdtext"><a href="discussion_topic.php?did=<?php echo $data['ThreadID'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')" title="Delete">Delete</a></td>
  </tr>
<?php	}?>
	<tr>
