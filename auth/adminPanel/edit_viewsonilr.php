...
if($_SESSION['logtoken']!=$_POST['random'])
		...
		$sql = "UPDATE `viewsonilr` SET `review_comment` = '$acomments', `review_date` = '$currentdt' WHERE `id`='$editid'";
		$ress = $conn->query($sql);
		...
		$sql_admin="select * from viewsonilr where id='$editid'";
		...
		$email_subject = "Views On ILR Submission"; // The Subject of the email
		$email_to= $email;
        ...
        <tr>
			<td colspan='3' align='left' valign='top'>Thankyou for your View On ILR. </td>
        </tr>
...
$msg="Views on ILR Comment has been sent successfully.";
...
exit;
 ...
 $edit="select * from viewsonilr where id='$editid'";
...
<title>Edit Feedback :: <?php echo $sitename; ?></title>
...
<div class="clear"></div>
 ...
 <h3 class="manageuser">Reply on ILR View</h3>
...
<input name="cmdsubmit" type="submit" class="button" id="cmdsubmit" value="Update" />&nbsp;
	<input name="cmdreset" type="reset" class="button" id="cmdreset" value="Reset" />&nbsp <input name="cid" type="hidden" value="<?php echo $fetch_result['id'];?>"/><input type="hidden" name="random" value="<?php echo $_SESSION['logtoken'];?>">&nbsp;<input type="button" class="button" value="Back" onClick="javascript:location.href = 'manage_viewsonilr.php';" />
	</span>
              <div class="clear"></div>
            </div>
</form>
