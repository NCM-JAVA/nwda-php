...
// $role_id_page=role_permission_page($user_id,$role_id,$model_id);
	...
	if(isset($cmdsubmit))
	{
		...
		if($errmsg == '')
		{
			...
			$sql = "UPDATE `feedback_form` SET `review_comment`='$acomments',`review_date`='$currentdt' WHERE id='$editid'";
			...
			@extract($line_admin1);
			$sql_admin="select * from feedback_form where id='$editid'";
			$rs1 = $conn->query($sql_admin); 
			$line_admin=$rs1->fetch_assoc();
			@extract($line_admin);
			...
			$email_from = 'noreply.nwda@gov.in'; // Who the email is from 
...
<div class="clear"></div>
 ...
 <h3 class="manageuser">Reply Feedback</h3>
