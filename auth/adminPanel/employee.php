...
if(isset($cmdsubmit) && $_GET['editid']=='')
{
	...
	if ($_FILES["txtuplodepdf"]["name"]==""){
		...
		if($errmsg == ''){
			...
			$sql = "INSERT INTO `employee`(`language_id`,`cat_id`,`m_name`,`m_short`,`m_title`,`m_content`,`page_url`,`start_date`,`end_date`,`approve_status`,`admin_id`,`c_new_status`,`image_file`,`create_date`,`docs_file`,`ext_url`,`content_type`,`m_keyword`,`m_description`,`module_id`)VALUES ('$txtlanguage','$cat_id','$txtename','$sortcontentdesc','$m_title','$txtcontentdesc','$url','$startdate','$expairydate','$txtstatus','$user_id','$newicons','$image_file','$createdate','$txtuplodepdf','$txtweblink','$texttype','$txtekeyword','$txtmeta_description','$category')";
			$res = $conn->query($sql);
...
/* 			$tableName="audit_trail";
			 ...
			 */
			$sql = "INSERT INTO `audit_trail` (`user_login_id`,`page_id`,`page_name`,`page_action`,`page_category`,`page_action_date`,`ip_address`,`lang`,`page_title`,`approve_status`)VALUES ('$user_id','$page_id','$txtename','$action','$model_id','$date','$ip','$txtlanguage','$txtepage_title','$txtstatus')";
	...
	if(isset($cmdsubmit) && $_GET['editid']!=''){
		...
		$txtekeyword 			= trim($_POST['txtekeyword']);
		$txtmeta_description 	= trim($_POST['txtmeta_description']);
		...
		if($errmsg == '')
		{
			...
			if ($_FILES["txtuplodepdf"]["name"]!=""){
				$sql = "select docs_file FROM employee WHERE m_id=$cid";
				$rs = mysql_query($sql);
			...
			$whereclause="m_id=$cid";
		/* 	$tableName_send="employee";
			$old=array("language_id","cat_id","m_name","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","c_new_status","create_date","ext_url","content_type","m_keyword","m_description","update_date","module_id");
			$new=array("$txtlanguage","$cat_id","$txtename","$sortcontentdesc","$m_title1","$txtcontentdesc","$url","$startdate","$expairydate","$txtstatus","$user_id","$newicons","$create_date","$txtweblink","$texttype","$txtekeyword","$txtmeta_description","$update_date","$category");
			$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);*/
			
			$sql1 = "UPDATE `employee` SET `language_id`='$txtlanguage',`cat_id`='$cat_id',`m_name`='$txtename',`m_short`='$sortcontentdesc',`m_title`='$m_title1',`m_content`='$txtcontentdesc',`page_url`='$url',`start_date`='$startdate',`end_date`='$expairydate',`approve_status`='$txtstatus',`admin_id`='$user_id',`c_new_status`='$newicons',`create_date`='$create_date',`ext_url`='$txtweblink',`content_type`='$texttype',`m_keyword`='$txtekeyword',`m_description`='$txtmeta_description',`update_date`='$update_date',`module_id`='$category' WHERE `m_id`=$cid";
...
$sqlimg = "SELECT * FROM employee WHERE m_id ='".$_GET['editid']."'";
$rsimg	 = $conn->query($sqlimg);
$imgg	 = $rsimg->fetch_array();
@extract($imgg);
...
<title>Manage Employee Document:-add/update: <?php echo $sitename; ?></title>
<!-- admin css  -->
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_employee.php" title="Manage Tenders">Manage Employee Document</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Add/Update Employee Document</span>
	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Add/Update Employee</h3>
...
<form action="" method="post" name="form1"  autocomplete="off" enctype="multipart/form-data"  onsubmit="return add_cp('form1')">
	...
	if($_GET['editid']!='')
	{
		$rq = "select * from employee where 	m_id='".$_GET['editid']."'";
		$rs = $conn->query($rq);
		$rr = $rs->fetch_array();
				//print_r($rr);
		}
...
<div class="frm_row"> <span class="label1">
              <label for="statename"> Select Module:</label>
              <span class="star">*</span></span> <span class="input1">
              <select name="category">
              <option>Select</option>
<?php 
$state_query = "SELECT `c_id`,`c_name`FROM `module_master`";
$state_res = $conn->query($state_query);
while($data = $state_res->fetch_array())
							{
@extract($data);
?>
<option value="<?php echo $c_id;?>" <?php if ($c_id==$rr['module_id']) echo 'selected="selected"';?>><?php echo $c_name;?></option>
<?php
							}
...
<div class="frm_row"> <span class="label1">
            ...
            <span class="star"></span></span> <span class="input1">
           <input type="file" name="txtuplodepdf" id="txtuplodepdf"/><?php echo $imgg['docs_file'];?>
            </span>
            <div class="clear"></div>
            </div>
<?php } else{?>
...
<?php } ?>
    <div class="frm_row"> <span class="label1">
                                          ...
                                          <input type="text" name="startdate" id="startdate" readonly="readonly"  autocomplete="off" value="<?php if($rr['start_date'] !=''){ echo changeformate($rr['start_date']); } else { } ?>"/><span class="date">[dd-mm-yyyy]</span> 
                                                ...
                                                <div class="frm_row"> <span class="label1">
                                                        <label for="expairydate">End Date:</label><span class="star">*</span>
                                                    </span> <span class="input1">
	...
	?>
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_employee.php';" />
         </span>
              <div class="clear"></div>
            </div>
