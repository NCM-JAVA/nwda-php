...
if($_SESSION['admin_auto_id_sess']=='')
	...
	if(isset($cmdsubmit) && $_GET['editid']=='')
	{
		...
		$title           = trim(str_replace("'", "\'", $_POST['txtetitle']));
		$sortcontentdesc = trim(str_replace("'", "\'", $_POST['sortcontentdesc']));
		$url             = seo_url($_POST['txtetitle']);
		...
		if($errmsg == ''){
			...
			$cat_id='1';
			//$tableName_send="combine";	//$tableFieldsName_old=array("language_id","cat_id","m_name","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","c_new_status","image_file","create_date");
			// $tableFieldsValues_send=array("$","$","$","$","$","$","$","$","$","$txtstatus","$","$","$","$");
			// $value=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);
        
        
       // $abc = "ALTER TABLE combine_publish MODIFY minister_thumb VARCHAR(255) NULL;";
        //$abc2 = $conn->query($abc);
        
			 $sql15 = "INSERT INTO `combine`(`language_id`, `cat_id`, `m_name`, `m_short`, `m_title`, `m_content`, `page_url`, `start_date`, `end_date`, `approve_status`, `admin_id`,`c_new_status`,`image_file`,`create_date`)VALUES('$txtlanguage','$cat_id','$txtename','$sortcontentdesc','$title','$txtcontentdesc','$url','$startdate','$expairydate','$txtstatus','$user_id','$newicons','$image_file','$create_date')";
			...
			// $tableName="audit_trail";	//$tableFieldsName_old=array("user_login_id","page_id","page_name","page_action","page_category","page_action_date","ip_address","lang","page_title","approve_status");	//$tableFieldsValues_send=array("$user_id","$page_id","$txtename","$action","$model_id","$date","$ip","$txtlanguage","$txtepage_title","$txtstatus");
      ...
      //$sql5 = "INSERT INTO `audit_trail`(`user_login_id`,`page_id`,`page_name`,`page_action`,`page_category`,`page_action_date`,`ip_address`,`lang`,`page_title`,`approve_status`)VALUES('$user_id','$deleteid','$pagename','$action','$model_id','$date','$ip','$txtlanguage','$gallery_categoryname','$txtstatus')";
			...
			$msg="What's New Added Successfully";
...
/* 				$image_path = "../../upload/photogallery/".$row['image_file'];
			...
			/* 	$tableName_send="combine";
				...
				$update = "UPDATE `combine` SET `docs_file`='$filename1' WHERE `m_id`=$cid";
			...
			$useAVclass->Up dateQuery($tableName_send,$whereclause,$old,$new);*/
   ...
   if (isset($_POST['txtcategory'])) {
    ...
    $txtcategory = 0; 
			...
			if($txtstatus=='3')
			{
				...
				$sql2="Select * from combine_publish $whereclause";
				$result1 = $conn->query($sql2);

				if($create_version==1 and $result1->num_rows >0){ 
...
<title>Manage What News add/update: <?php echo $sitename;?></title>
<!-- admin css  -->
...
<form action="" method="post" name="form1"  autocomplete="off" enctype="multipart/form-data"  onsubmit="return add_cp('form1')">
	<?php	
	if($_GET['editid']!=''){
		$rq="select * from combine where m_id='".$_GET['editid']."'";
		$rs = $conn->query($rq);
		$rr = $rs->fetch_array();			
	}
?>   
			...
			<div class="frm_row"> <span class="label1">
              ...
              <span class="star">*</span></span> <span class="input1">
              <textarea rows="2" cols="35" name="sortcontentdesc" autocomplete="off"  id="sortcontentdesc" onkeyup="Javascript:charactersRemaining(this.form.sortcontentdesc, 500, this.form.msg_left); characterLimit(this.form.sortcontentdesc, 500, this.form.sortcontentdesc);" onkeypress="Javascript:charactersRemaining(this.form.sortcontentdesc, 500, this.form.msg_left); characterLimit(this.form.sortcontentdesc, 500, this.form.sortcontentdesc);" onmouseout="Javascript:charactersRemaining(this.form.sortcontentdesc, 500, this.form.msg_left); characterLimit(this.form.sortcontentdesc, 500, this.form.sortcontentdesc);" tabindex="1" ><?php if($sortcontentdesc!=""){ echo $sortcontentdesc;}else{ echo $rr['m_short'];} ?>
              ...
              <div class="clear"></div>
			    ...
			    <?php if($_GET['editid']!=''){ ?>
                        <div class="frm_row">
                           ...
                           <span class="star"></span></span> <span class="input1">
                           <input type="file" name="txtuplode" id="txtuplode"/><a href="<?php echo $HomeUrl; ?>/upload/<?php echo $rr['image_file']; ?>">View File</a>
                           </span>
                           <div class="clear"></div>
                        </div>
                        <?php } else{?>
 ...
 <div class="frm_row"> <span class="label1">
        ...
        <div class="frm_row"> 
			...
			if($user_id =='101')
			{
			$sql0="select * from content_state where state_status=1";
			$rss = $conn->query($sql0);
