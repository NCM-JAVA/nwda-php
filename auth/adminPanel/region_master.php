...
if(isset($_GET['id']) && $_GET['id']!="" )
{
	...
	$rm_id = mysql_real_escape_string( $_GET['id'] );
	$rm_id = check_input($rm_id);
...
if(isset($_POST['Submit_g']) && $rm_id=='')
{
	if($_SESSION['logtoken']!=$_POST['random'])
	{
		...
		if($errmsg=='')
		{
			...
			$msg=CONTENTADD;
			$_SESSION['SESS_MSG']=$msg;
			header("location:region_master.php");
			exit;
...
if($_GET['did']!='')
{
	if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("delete from region_master where c_id='$did'");
...
<head>
	...
	<title>Manage Region Master : <?=$sitename; ?></title>
		...
		<div class="right_col1">
			...
			<div class="addmenu">
				<div class="internalpage_heading">
					<h3 class="manageuser">Manage Region </h3>
			...
			-->
				...
				<div class="grid_view">
					<form action="" method="post" enctype="multipart/form-data" style="margin:0px; padding:0px;">
						...
						{
							$rq = mysql_query("select * from region_master where c_id='".$rm_id."'");
							//echo "select * from region_master where c_id='".$rm_id."'";
            ...
            <input name="cmdreset" type="reset" class="button" id="cmdreset" value="Reset" />
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'manage_photogallery.php';" />
					...
					<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc">
						...
						<?php
						$columns = "select * ";
						$sql = "from region_master where 1 ";
						...
						<?php	}else	{	?>
							...
							while($data=mysql_fetch_array($rows)){
								...
								<tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
									...
									<td width="47" align="center" class="left-tdtext"><a href="region_master.php?id=<?php echo $data['c_id'];?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>

									<td width="63" align="center" class="left-tdtext"><a href="region_master.php?did=<?php echo $data['c_id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
								</tr>
							<?php	}?>
							<tr>
