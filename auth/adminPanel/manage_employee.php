...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
				...
				$sqd = "select image_file FROM employee WHERE m_id=$deleteid";
				$sql="Delete From employee where m_id='$deleteid'";
				...
				$row = $rs->fetch_array();
				   ...
				   $Query = $conn->query($SQL1);
					...
					// $pagename  = mysqli_result($Query,0,'page_name');
					// $txtlanguage  = mysql_result($Query,0,'');
					// $txtstatus  = mysql_result($Query,0,'');
					// $gallery_categoryname  = mysql_result($Query,0,'');
					...
					if($res)
					{	
					header("location:delete.php?status=employees_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
	...
	$sql="Update employee set approve_status='1' where m_id='$inactiveid'";
	$res1=$conn->query($sql);
	$sql1="Update employee_publish set approve_status='1' where m_publish_id='$inactiveid'";
	$res=$conn->query($sql1);
	if($res)
	{	
	header("location:delete.php?status=employees_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage  Employee Document: <?php echo $sitename;?></title>
...
<?php 
	if($btnsubmit=="Search")
	{
		...
		if($errmsg==''){
			$query ="select * from employee where cat_id =4 $querywhere ORDER BY page_postion ASC "; 
		}
		else{
		...
		$query ="select * from employee where cat_id =4 $wherecluse ORDER BY page_postion ASC ";
	}
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Employee Document</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="employee.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_employee.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('employeeview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=employee'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="employee.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_employee.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
