...
if(isset($_POST['Submit_g']) && $_GET['id']=='')
{
    ...
    if(trim($c_name)=="")
	{
		$errmsg .="Please enter category name."."<br>";
	}
	...
	if(trim($user_status)=="Select")
	{
		...
		if($errmsg=='')
		{
			$tableName_send="category_master";
...
if($_GET['did']!='')
{
	if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysql_query("DELETE FROM `category_master`where c_id='$did'");
...
<head>
	...
	<title>Manage Category Master : <?=$sitename; ?></title>
		...
		<div class="right_col1">
			...
			<div class="addmenu">
				<div class="internalpage_heading">
					<h3 class="manageuser">Manage Category </h3>
				...
				<div class="grid_view">
					<form action="" method="post" enctype="multipart/form-data" style="margin:0px; padding:0px;">
						...
						if($_GET['id']!='')
						{
						     $rq = mysql_query("select * from  category_master where c_id='".$_GET['id']."'");
...
?>
              ...
              <label for="categoryename"> Category Name:</label>
              ...
              <input name="c_name" type="text" size="50" class="input_class" id="" value="<?php echo html_entity_decode($rr['c_name']);?>" />
						...
						<div class="frm_row"> <span class="label1">
              <label for="statenamehi">Category Hindi Name:</label>
             ...
             <input type="button" class="button" value="Back" onClick="javascript:location.href = 'State_Master.php';" />
					...
					<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc">
						<tr bgcolor="whitesmoke">
							...
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Category Name</span></th>
							<th width="510" align="left" valign="top" bgcolor="whitesmoke" class="topheader_td"><span class="left-text">Category Name (Hindi)</span></th>
						...
						<?php
						$sql="SELECT * FROM `category_master` order by `c_name` ";
						...
						<?php	}else	{	?>
							...
							while($data=mysql_fetch_array($rows)){
								...
								<tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
									...
									<td width="47" align="center" class="left-tdtext"><a href="category_master.php?id=<?php echo $data['c_id'];?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>

									<td width="63" align="center" class="left-tdtext"><a href="category_master.php?did=<?php echo $data['c_id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
								</tr>
							<?php	}?>
							<tr>
