...
$role_id=$_SESSION['dbrole_id'];
 ...
 $model_id='Manage Consultation Paper';
 //$module_id='Manage What News';
...
$tableName_send="schemes";

$tableFieldsName_old=array("language_id","m_name","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","image_file","create_date","docs_file","ext_url","content_type","m_keyword","m_description");
$tableFieldsValues_send=array("$txtlanguage","$txtename","$sortcontentdesc","$m_title","$txtcontentdesc","$url","$startdate","$expairydate","$txtstatus","$user_id","$image_file","$createdate","$txtuplodepdf","$txtweblink","$texttype","$txtekeyword","$txtmeta_description");
$value=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);
$page_id=mysql_insert_id();
if($txtstatus=='3')
{
	/*$tableName_send="schemes_publish";
	...
	$sql="INSERT INTO schemes_publish (m_publish_id,language_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,create_date,image_file,docs_file,ext_url,content_type,m_keyword,m_description)
	 SELECT m_id, language_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,create_date,image_file,docs_file,ext_url,content_type,m_keyword,m_description
   FROM schemes WHERE m_id=$page_id";
  mysql_query($sql);
}
 ...
 if ($_FILES["txtuplode"]["name"] != "")
	 {
			...
			$image_source = imagecreatefromjpeg($_FILES["txtuplode".$i]["tmp_name"]);
			  ...
			  $sql = "select image_file FROM schemes WHERE m_id=$cid";
    $rs = mysql_query($sql);
   ...
   unlink($image_path2);	
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
			...
			$sql = "select docs_file FROM schemes WHERE m_id=$cid";
			$rs = mysql_query($sql);
			$row=mysql_fetch_array($rs);
...
$create_date=date('y-m-d');
	$whereclause="m_id=$cid";
$tableName_send="schemes";
$old=array("language_id","m_name","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","create_date","ext_url","content_type","m_keyword","m_description");
$new=array("$txtlanguage","$txtename","$sortcontentdesc","$m_title1","$txtcontentdesc","$url","$startdate","$expairydate","$txtstatus","$user_id","$create_date","$txtweblink","$texttype","$txtekeyword","$txtmeta_description");
	$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
if($txtstatus=='3')
{
	$tableName_send="schemes_publish";
	$whereclause="where m_publish_id='$cid'";
$sql=mysql_query("Select * from schemes_publish $whereclause");
$row=mysql_num_rows($sql); 
	if($row >0)
	{
 $update="UPDATE schemes_publish,schemes SET schemes_publish.m_publish_id = schemes.m_id,schemes_publish.language_id = schemes.language_id,schemes_publish.m_name = schemes.m_name,schemes_publish.m_short = schemes.m_short,schemes_publish.m_title = schemes.m_title,schemes_publish.m_content = schemes.m_content,schemes_publish.page_url = schemes.page_url,schemes_publish.start_date = schemes.start_date,schemes_publish.end_date = schemes.end_date,schemes_publish.approve_status = schemes.approve_status,schemes_publish.admin_id = schemes.admin_id,schemes_publish.create_date = schemes.create_date,schemes_publish.image_file = schemes.image_file,schemes_publish.docs_file = schemes.docs_file,schemes_publish.ext_url = schemes.ext_url,schemes_publish.content_type = schemes.content_type,schemes_publish.m_keyword = schemes.m_keyword,schemes_publish.m_description = schemes.m_description WHERE schemes_publish.m_publish_id =schemes.m_id and schemes.m_id=$cid";
  mysql_query($update);
	}
	else {
$sql="INSERT INTO schemes_publish (m_publish_id,language_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,create_date,image_file,docs_file,ext_url,content_type,m_keyword,m_description)
	 SELECT m_id, language_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,create_date,image_file,docs_file,ext_url,content_type,m_keyword,m_description
   FROM schemes WHERE m_id=$cid";
  mysql_query($sql);
  }
}
		$user_id=$_SESSION['admin_auto_id_sess'];
...
<title>Manage schemes add/update: <?=$sitename;?></title>
<!-- admin css  -->
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_schemes.php" title="Manage Circular/Events/Tenders">Manage Schemes</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Add/Update Schemes</span>
	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Add/Update Schemes</h3>
...
<form action="" method="post" name="form1"  autocomplete="off" enctype="multipart/form-data"  onsubmit="return add_cp('form1')">
	...
	if($_GET['editid']!='')
	{
		$rq = mysql_query("select * from schemes where 	m_id='".$_GET['editid']."'");
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
	...
	?>
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_schemes.php';" />
         </span>
              <div class="clear"></div>
            </div>
