...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM important_link WHERE m_id=$cid";
					$sql="Delete From important_link where m_id='$deleteid'";
					$res=mysql_query($sql);
   ...
   unlink($image_path2);	
					...
					if($res)
					{	
					header("location:delete.php?status=important_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update important_link set approve_status='1' where m_id='$inactiveid'";
 $res=mysql_query($sql);
  $sql="Update important_link_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysql_query($sql);
	if($res)
	{	
	header("location:delete.php?status=important_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Important Link: <?php echo $sitename; ?></title>
...
<?php 
	if($btnsubmit=="Search")
	{
		...
		if($textcategory !='')
		{
		$querywhere .=" and category_type=$textcategory"; 
		}
...
if($txtstatus=='')
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
  $query ="select * from important_link where 1 $querywhere ORDER BY page_postion ASC ";  
	  }
	 else
	{
...
$query ="select * from important_link where 1 $wherecluse ORDER BY page_postion ASC ";
}
?>      
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Important Link</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="important_link.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
{
	?>
<option value="<?php echo $key; ?>" <?php if($key==$textcategory){ echo 'selected="selected"'; } else { }?>><?php echo $value; ?></option>
<?php }
 ?>
</select>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_important_link.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('importantview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=important'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="important_link.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_important_link.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
