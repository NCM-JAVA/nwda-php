...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			...
			if($check_status >0)
			{
					...
					$sql = "Delete From home_page where m_id='$deleteid'"; 
					$res = $conn->query($sql);
					$page_id = $conn->insert_id; 
				
					$sqd = "select image_file FROM home_page WHERE m_id=$deleteid"; 
					$rs  = $conn->query($sqd);
					$row = $rs->fetch_array();

				 	$sql3 = "Delete From home_page_publish where m_publish_id='$deleteid'"; 
					$res = $conn->query($sql3);
					...
					if($res)
					{						
						header("location:delete.php?status=home_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update home_page set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn, $sql);
  $sql="Update home_page_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysqli_query($conn, $sql);
	if($res)
	{	
	header("location:delete.php?status=home_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Home Page: <?php echo $sitename;?></title>
...
<?php 
	if($btnsubmit=="Search")
	{
		...
		if($hometype!='')
		{
					$querywhere .=" and m_type=$hometype"; 
		}
		if($txtstatus=='')
		{
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
  $query ="select * from home_page where 1 $querywhere"; 
	  }
	 else
	{
...
$query ="select * from home_page $wherecluse ";
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Home Page</h3>
 <div class="right-section">

			 <ul>
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="homepage.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<input id="filter_search" type="text" title="Search title or Menu Name." value="" name="filter_search"><select name="hometype" id="hometype"  autocomplete="off">
			<option value=""> Select </option> 	
			<?php  foreach($homepage_type as $key=>$value){?>
				<option value="<?php echo $key;?>" <?php if($hometype==$key){ echo 'selected'; } ?>> <?php echo $value;?> </option>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_homepage.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('homepageview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=profile'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="homepage.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_homepage.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
