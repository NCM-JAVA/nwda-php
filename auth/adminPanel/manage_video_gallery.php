...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					if($res)
					{	
					header("location:delete.php?status=video_gallerydelete");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
 ...
 $res=mysql_query($sql);
	if($res)
	{	
	header("location:delete.php?status=video_gallery");
	}
}
?>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Video Gallery: <?php echo $sitename;?></title>
...
<?php if($btnsubmit=="Search")
{
		...
		if($menucategory!='')
		{
				...
				$errmsg = 'Please select correct field.<br>';
				 ...
				 {
					$querywhere .=" and gallery_type=$menucategory"; 
				 }
		}
		if($txtstatus=='')
		{
	  ...
	  if($errmsg=='')	
	  {
		 $query ="select * from photogallery  where gallery_type='2'  and approve_status='3' $querywhere  ORDER BY page_position ASC";
	  }
	 else
	{
...
$wherecluse="where gallery_type='2' and approve_status='$txtstatus'";
}
$query ="select * from photogallery $wherecluse and gallery_type='2'    ORDER BY page_position ASC";
}
?>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Video Gallery</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="videogallery.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select name="txtstatus" id="txtstatus"  class="inputbox" autocomplete="off">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<span class="space-menuname_pg"><input type="radio"  name="radio1" id="<?php echo $id; ?>"  value="<?php echo $id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $sortdesc	; ?></span>
							<span class="space-menuname_mm"><?php echo $sort_desc_hindi; ?></span>
							
							<span class="space-pagestatus"><?php status($approve_status);?></span>
								<span class="space-option">
                          <?php   if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_video_gallery.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('photoview.php?page_id=<?php echo $id;?>','window','width=700,height=600,scrollbars=yes')">View </a> 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="videogallery.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_video_gallery.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
