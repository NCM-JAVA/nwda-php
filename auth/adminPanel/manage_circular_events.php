<?php ob_start();
	...
	if($deleteid !=''){
		...
		if($check_status >0){
			...
			$sqd = "select image_file FROM combine WHERE m_id=$deleteid";
			$sql="Delete From combine where m_id='$deleteid'";
			$res1 = $conn->query($sql);
			 
			$rs  = $conn->query($sqd);
			$row = $rs->fetch_array();
			
			$sql1="Delete From combine_publish where m_id='$deleteid'";
			$res  = $conn->query($sql1);
...
/* 
			...
			unlink($image_path);
			unlink($image_path2);	 */
			$SQL2 = "SELECT * FROM audit_trail where page_id='$deleteid'";
			$Query = $conn->query($conn, $SQL2);
...
/* 			$tableName="audit_trail";
			...
			if($res){	
				header("location:delete.php?status=circualr_deleteid");
			}
		}else{
			/*session_unset($admin_auto_id_sess);
			session_unset($login_name);
	...
	if(($inactiveid !='')){
		...
		$sql1="Update combine set approve_status='1' where m_id='$inactiveid'";
		$res1=mysqli_query($conn, $sql1);
		$sql2="Update combine_publish set approve_status='1' where m_publish_id='$inactiveid'";
		$res2=mysqli_query($conn, $sql2);
		if($res2){	
			header("location:delete.php?status=circualr_inactiveid");
		}
	}
?>
				...
				<div class="right_col1">
					...
					<?php 
						if($btnsubmit=="Search"){
							...
							if($errmsg==''){
								$query ="select * from combine where 1 and(cat_id !=1 and cat_id !=4) $querywhere ORDER BY end_date DESC  "; 
							}else{
							...
							$query ="select * from combine where 1 and(cat_id !=1 and cat_id !=4) $wherecluse ORDER BY end_date DESC ";
						}
					...
					<div class="internalpage_heading">
						<h3 class="manageuser">Manage Circular / Events / News</h3>
						<div class="right-section">
							<ul>
							<?php if($role_map['draft']=='DR' || $user_id=='101'){?>
								<li class="new-icon"><a href="circular_events.php" title="New"><span class="icon-28-new"></span>New</a></li>
							<?php }?>
								...
								<form id="manage_menu" name="manage_menu" method="post" action="">
									<div class="filter-select fltrt">          
										...
										<?php foreach($cat_type as $key=>$value){ ?>
											<option value="<?php echo $key; ?>" <?php if($key==$texcat){ echo 'selected="selected"'; } else { }?>><?php echo $value; ?></option>
										<?php } ?>
										</select>
										...
										<?php } if($role_map['draft']=='DR' || $user_id=='101' || $user_id=='104'){ ?>
									...
									<ul class="">
										...
										<li id="arrayorder_<?php echo $m_id ?>" class="<?php echo $class;?>">
											<span class="space-menuname_pg">
												<input type="radio" name="radio1" id="<?php echo $m_id; ?>"  value="<?php echo $m_id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $m_name; ?>
											</span>					
											...
											<?php  if($role_map['publish']=='PB' || $txtstatus=='3'){?>
												<a href="manage_circular_events.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
												<a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('manageview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
											</span>
											<div class="clear"></div>
										</li><?php }?>
									</ul>
    ...
    jQuery.ajax({
        ...
        success: function (pub) { 
			...
			if(dataid==0){
				...
				var e='<a href="circular_events.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
				var d='<a href="manage_circular_events.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
