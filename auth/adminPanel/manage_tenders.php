...
// $role_id_page=role_permission_page($user_id,$role_id,$model_id);
	...
	if($deleteid !=''){
		...
		if($check_status >0){
			...
			$sqd = "select image_file FROM tender WHERE m_id='$deleteid'";
			$sql="Delete From tender where m_id='$deleteid'";
			$res1 = $conn->query($sql);
			 
			$rs  = $conn->query($sqd);
			$row = $rs->fetch_array();
			
			$sql1="Delete From tender_publish where m_id='$deleteid'";
			$res  = $conn->query($sql1);
...
/* 			$sql_q = "INSERT INTO audit_trail ('user_login_id','page_id','page_name','page_action','page_category','page_action_date','ip_address','lang','page_title','approve_status')VALUES ('$user_id','$deleteid','$pagename','$action','$model_id','$date','$ip','$txtlanguage','$gallery_categoryname','$txtstatus')";
				$sqli1 = $conn->query($sql_q); */

			if($res){	
				header("location:delete.php?status=tenders_deleteid");
			}
		}
		else{
			/*session_unset($admin_auto_id_sess);
	...
	if(($inactiveid !='')){
		...
		$sql1="Update tender set approve_status='1' where m_id='$inactiveid'";
		$res1=mysqli_query($conn, $sql1);
		$sql2="Update tender_publish set approve_status='1' where m_publish_id='$inactiveid'";
		$res2=mysqli_query($conn, $sql2);
		if($res2){	
			header("location:delete.php?status=tenders_inactiveid");
		}
	}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Manage  Tenders : <?php echo $sitename;?></title>
				...
				<div class="right_col1">
					...
					<?php 
						if($btnsubmit=="Search"){
							...
							if($errmsg==''){
								$query ="select * from tender where cat_id =4 $querywhere ORDER BY start_date DESC"; 
							}else{
							...
							$query ="select * from tender where cat_id =4 $wherecluse ORDER BY start_date DESC"; //page_postion ASC 
						}
					...
					<div class="internalpage_heading">
						<h3 class="manageuser">Manage Tenders</h3>
						<div class="right-section">
							<ul>
								<?php if($role_map['draft']=='DR' || $user_id=='101'){?>
								<li class="new-icon">
									<a href="tenders.php" title="New"><span class="icon-28-new"></span>New</a>
								</li>
								<?php }?>
								<?php if($role_map['medit']=='ED' || $user_id=='101'){?>
									<li id="editer" class="edit-icon"><a href="#" onclick="alert('Please select atleast one record')" title="Edit">Edit</a><?php }?></li>
							...
							<div class="grid_view">
								...
								<div id="list">
									...
									<ul class="">
										...
										?>
										<li id="arrayorder_<?php echo $m_id ?>" class="<?php echo $class;?>">
											<span class="space-menuname_pg"><input type="radio"  name="radio1" id="<?php echo $m_id; ?>"  value="<?php echo $m_id; ?>" onclick="editlist(this.value);" >&nbsp;&nbsp; <?php echo $m_name; ?></span>				
											...
											<a href="manage_tenders.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
											<a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('tenderview.php?page_id=<?php echo $m_id;?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
											</span>
											<div class="clear"></div>
										</li><?php }?>
									</ul>
									<ul>
										<li class="page" style="text-align:center;"><?php echo $pager->renderFullNav();?></li>
									</ul>
									<?php } else{ ?>
									<ul> 
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=tender'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="tenders.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_tenders.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
