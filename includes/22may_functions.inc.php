...
$body=str_replace($match[0],$newphrase,$body);
								...
								if($startstr=="http")
										...
										$body=str_replace($match[0],"<a  href='$match[2]'  onclick='return sitevisit()' target='_blank'  title='$match[3]'>$match[3]&nbsp;<img src='$HomeURL/images/pdf_icon.png' alt='pdf' ></a>",$body);
											...
											if(($exten=='xlsx') || ($exten=='.xls'))
										   {
											...
											else
											{
												...
												$body=str_replace($match[0],"<a  href='$match[2]'   target='_blank'  title='$match[3]'>$match[3]&nbsp;<img src='$HomeURL/images/external_link_icon.png' alt='external link'></a>",$body);
											}
									...
									else
									{
											...
											$f = $_SERVER['DOCUMENT_ROOT'].str_replace(  '%20',' ' , str_replace('http://www.servicetax.gov.in' , '' , $match[2]) );
...
function change_date_full_formate($date)
	...
	function front_email_check_exit($txtename,$field_name,$tableName_send)
	 {
    $sql="select * from ".$tableName_send." where ".$field_name."='".$txtename."' and user_status='1' and ( user_type='2' or user_type='3' or user_type='4' or user_type='6')";
 	   $rs=mysql_query($sql);
	   $result_rows=mysql_num_rows($rs);
	   		if($result_rows >0){
			return 0;
