...
//echo "SELECT m_flag_id as page_id,m_publish_id FROM schemes_publish where m_publish_id ='$page_id' and 	approve_status='3' ORDER BY page_postion ASC";
	...
	if ($pag==0) return $upload_img;
	else return get_page_img($pag);
}
...
function func_org_designation($oldID) {
	$sql = mysql_query("select * from org_setup where 	deg_id='$oldID'");
...
function func_org_designationone($oldID) {
	$sql = mysql_query("select * from org_setup where approve_status='3' and deg_id='$oldID'");
	//echo "select * from org_designation where approve_status='3' and deg_id='$oldID'";
	$row = mysql_fetch_array($sql);
	return $row['designation'];
}
...
function getAllBlogList($blogid =''){
         //echo "select * from tbl_blog where 1";
         $records = array();
         if($blogid !=''){
 ...
 function getCategories($cat_type){
	//echo "Select * from manage_category where category_type = '$cat_type' and c_status=3";
...
function infoname($oldID) {
	...
	$result = mysql_query("Select * from module_master where c_id='$oldID'"); 
		$line = mysql_fetch_array($result);
		return $line['c_name'];
}
...
function getJobCategories($cat_type){
	//echo "Select * from manage_jobcategory where category_type = '$cat_type' and c_status=1";
	$sql = mysql_query("Select * from manage_jobcategory where category_type = '$cat_type' and c_status=1");	
	if(mysql_num_rows($sql) > 0){
		while($row = mysql_fetch_assoc($sql)){
			$data[] = $row;
		}
...
function getJobCategoryById($id){
	$sql = mysql_query("Select * from manage_jobcategory where c_id = '$id'");	
	if(mysql_num_rows($sql) > 0){
		while($row = mysql_fetch_assoc($sql)){
			$data = $row;
		}
...
function getJobLocationById($id){
	$sql = mysql_query("Select * from state where state_id = '$id'");	
	if(mysql_num_rows($sql) > 0){
		while($row = mysql_fetch_assoc($sql)){
			$data = $row;
		}
...
function getJobTitleById($id){
	//echo "Select * from manage_category where category_type = '$cat_type' and c_status=3";
	echo $sqldata="Select * from manage_vacancy where id = '$id'";
