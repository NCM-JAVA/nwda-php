...
<html lang="en">
	<head>
		...
		<meta name="description" content="NWDA sitemap shows all page related to our website">
	...
	<section>
			...
			<div class="container"  id="skipCont">
				<div class="row">
					...
					<?php
			    $sqlquery="select * from menu_publish where m_flag_id='0' and (menu_positions='1' OR menu_positions='2' OR menu_positions='3')  and language_id='1' and approve_status='3'  ORDER BY page_postion ASC";
				$sql = mysqli_query($conn, $sqlquery); 
			$count=mysqli_num_rows($sql);
				$i = 1;
				while ($row = mysqli_fetch_array($sql)) {
				$sqlquery11="select * from menu_publish where m_flag_id='".$row['m_publish_id']."' and language_id='1' and approve_status='3' ORDER BY page_postion ASC";
                ...
                $sqlquery22="select * from menu_publish where m_flag_id='".$rows['m_publish_id']."' and language_id='1' and approve_status='3' ORDER BY page_postion ASC";
				$sql2 = mysqli_query($conn, $sqlquery22);
			    $row3 = mysqli_num_rows($sql2);
				$m_id=$row2['m_id']; 
$url = 	$HomeURL.'/content/innerpage/'.$rows['m_url'];                    
					 ?>
						<li class="<?php if($row3>0) { ?>has-sub<?php  } ?>"><a title="<?php echo $rows['m_name'];?>" href="<?php if($rows['m_url']=='.php'){ echo $rows['linkstatus']; }else{ echo $url; } ?>"><?php echo  $rows['m_name'];?></a>
						<?php if($row3>0 ) {  ?>
						<ul>
						<?php while ($rowssub = mysqli_fetch_array($sql2)) {  ?>
								<li><a title="<?php echo $rowssub['m_name'];?>" href="<?php echo $HomeURL;?>/content/innerpage/<?php echo $rowssub['m_url']; ?>"><?php echo  $rowssub['m_name'];?></a></li>
						<?php }  ?>		
						</ul>
                        <?php  }  ?>							
						</li>
