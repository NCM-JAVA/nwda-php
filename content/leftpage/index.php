		...
		if($url !='')
		{
		 ...
		 $sql="SELECT m_publish_id as page_id, m_flag_id as m_flag_id , m_name, content as content, m_url ,m_title,menu_positions,m_keyword,m_description,upload_img FROM menu_publish where language_id='1' and approve_status='3'";
			...
			$m_name=$line3['m_name']; 
			 ...
			 if($row['upload_img']=="")
			 	$row['upload_img'] = '594264cff26ffwater_banner.jpg';
					...
					$regexp = "<a\s[^>]*href=(\"??)([^\" >]*?)\\1[^>]*>(.*)<\/a>"; 
						...
						if($startstr=="http")
						{
							...
							//echo $file = $_SERVER['HTTP_HOST']. $match[2];
								$body=str_replace($match[0],"<a  href='$match[2]'  onclick='return sitevisit()' target='_blank'  title='PDF that opens in new window'>$match[3]&nbsp;<img src='$HomeURL/images/pdf_icon.png' alt='PDF File'>".'&nbsp;&nbsp;'  .'<span class="file-size">'. Filebytes($file).'</span></a>',$body);
							}
						...
						else
						{
							...
							$file = $_SERVER['DOCUMENT_ROOT'].str_replace(  '%20',' ' , str_replace('https://www.servicetax.gov.in' , '' , $match[2]) );
								...
								$body=str_replace($match[0],"<a  href='$match[2]' target='_blank'  title='PDF that opens in new window'>$match[3]&nbsp;<img src=' $HomeURL/images/pdf_icon.png' width='12' height='12' alt='PDF File' />".'&nbsp;&nbsp;'  .'[<span class="file-size">'. Filebytes($file).'</span>]</a>',$body);
							}
