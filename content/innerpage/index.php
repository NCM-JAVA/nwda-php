...
// include("../../counter.php");
	...
	if($_SERVER['REQUEST_URI']){ 
		...
		if($m_url=='organization-chart.php'){
			header("Locati on:".$HomeURL."/content/organization.php");
			exit();
		}
...
<html lang="en">
	...
	<body id="fontSize">
			<header><?php include("../top_bar.php");?> </header>
			...
			<div class="container" id="skipCont">
				<div class="row">
					...
					<div class="col-sm-9 main-content inner">
						...
						$url=$val['3'];
						$open=$val['2'];
						if($url=='vacancydetails.php'){?>
						<div><a href="./vacancy-archives.php" class="archive">View Archive</a></div>
						<?php }elseif($url=='ilr-in-media.php'){ ?>
						<div><a href="./water-resources-in-media-archive.php" class="archive">View Archive</a></div>
						<?php } ?>
						<div class="content_ff">
							...
							if(preg_match_all("/$regexp/siU", $body, $matches, PREG_SET_ORDER)){ 
								foreach($matches as $match){ 
									...
									if($startstr=="http"){
										if($exten=='.pdf'){
											$file = $_SERVER['DOCUMENT_ROOT'].str_replace(  '%20',' ' , str_replace('https://www.servicetax.gov.in' , '' , $match[2]) );
										...
										if(($exten=='.doc') || ($exten=='.docx')){
											...
											if($match[3]=='eOffice NWDA - via NIC'){
												$body=str_replace($match[0],"<a  href='$match[2]'  onclick='return sitevisit1();' target='_blank'  title='External Link thats open in new window'>$match[3]&nbsp;<img src='$HomeURL/images/extlink.png' alt='External link'></a>",$body);
											}else{
