...
<html lang="en">
	<head>
		<title>Related Links: National Water Development Agency</title>
		<meta charset="utf-8">
				...
				<div class="row">
					...
					<div class="">
						...
						<div class="demo5 demof">
										<ul class="list-group">
						<?php  $sqlnews="select * from  important_link_publish where category_type='1' and approve_status='3' order by m_id asc ";
                    $resnews=mysql_query($sqlnews) or die(mysql_error());
					while($rownews=mysql_fetch_array($resnews))
					{
					
			if($rownews['content_type']=='0'){  $pageurlh1='<a title="'.$rownews['m_name'].'" target="_blank" href="'.$rownews['ext_url'].'">'.$rownews['m_name'].'</a>'; }
                 		
				?>	
				<li style="margin-left:50px;"><?php echo $pageurlh1;?></li>
			  <?php }  ?>		
</ul></div>			  
					</div>
				</div>
