...
<html lang="en">
   <head>
      <title>Vacancy Archives: National Water Development Agency</title>
      <meta charset="utf-8">
            ...
            <div class="row">
               ...
               <div class="col-sm-9 main-content inner">
                  ...
                  <div class="archive-grid">
                     <form  action="vacancy_archives.php" name="searchform" method="post">
                        <div class="acchive-div">
							<div class="archive-div">
								<label for="texcat" style="position: absolute;">Vacancy Title</label>
					 ...
					 <table class="table table-bordered table-responsive" >
                        ...
                        <?php 	
                           ...
                           $expairydate1=content_desc($expairydate1);
							...
							if($txtname!=""){
								$querywh=" and `job_title` LIKE '%$txtname%'"; 
                            }
                           
							
							if($errmsg == ''){
								$date=date('Y-m-d');
                                $query="SELECT * from manage_vacancy where `status`=3 and date(end_date ) < '$date' and `language_id`=1 $querywhere"."$querywh";
                           ...
                           {    
								...
								$date=date('Y-m-d'); 
								$query="SELECT * from manage_vacancy where `status`=3 and date(end_date) < '$date' and `language_id`=1 $querywhere   ";
								$res=$conn->query($query);
								$key=1;
								if($res->num_rows >0){
								while($row=$res->fetch_array()) 
