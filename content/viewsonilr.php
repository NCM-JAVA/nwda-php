...
if(isset($cmdsubmit))
{
	...
	$address =content_desc(trim($_POST['address']));
	...
	if(trim($txtcomment)=="")
	{
		$errmsg .="Please enter Views.<br>";
	}
	else if(preg_match("/^[aA-zZ0-9][a-zA-Z0-9 -.]{2,200}+$/", $txtcomment) == 0)
	{
	$errmsg .= "Please enter Alphanumeric Characters only, minimum 3 and maximum 200."."<br>";
	}   
...
/*  			$tableName_send="viewsonilr";
			$tableFieldsName_send=array("name","email","phone","address","comments","create_date","module_id");
			$tableFieldsValues_send=array("$txtename","$txtemail","$txtphone","$address","$txtcomment","$create_date","");
			$useAVclass->insertQuery($tableName_send,$tableFieldsName_send,$tableFieldsValues_send); 	 */		
			$sql= "INSERT INTO `viewsonilr`(`name`, `email`, `phone`, `address`, `comments`,`create_date`, `module_id`) VALUES ('$txtename','$txtemail','$txtphone','$address','$txtcomment','$create_date','')";
			$rss = $conn->query($sql);
		...
		$to= $txtemail;
		$from=$user_email;
		$subject = "User ILR Notification"; // The Subject of the email
		...
		<tr><td colspan='3' class='text_mail'>Thank you for sharing your View on ILR with us. We will follow up with you regarding your questions, concerns or comments as soon as possible.</td></tr>
...
mail($to, $subject, $message1, $headers);
		...
		$email_subject = "Views On ILR"; // The Subject of the email
		$email_to= $user_email;
		$headers  = "MIME-Version: 1.0".$eol;
		$headers .= "Content-Type: text/html; charset=UTF-8".$eol;
		$headers .= "From: ".$to."".$eol;
		...
		<tr>
			<td align='left' valign='top'><strong>View on ILR Date</strong></td>
			<td align='left' valign='top'><strong>:</strong></td>
		...
		$msg="Your View on ILR Submitted Successfully.";
		$_SESSION['sess_msg']=$msg;
		echo $_SESSION['sess_msg'];
		header("location:viewsonilr.php");
		exit;
...
<html lang="en">
	<head>
		<title>Views on ILR : <?= $sitename; ?></title>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="title" content="Views On ILR">
<meta name="description" content="For providing views/opinion on ILR related matter">
<meta name="keywords" content="View of interlinking of rivers, ILR">
			...
			<div class="container"  id="skipCont">
				<div class="row">
					...
					<div class="">
						...
						<h2>Views on ILR</h2>
                     <form action="viewsonilr.php"  method="post" name="form1" id="viewsonilr-form"   autocomplete="off">
					<?php if($errmsg!=""){?>
					...
					<div align="left"><font color="green"><h3>Your View on ILR Submitted Successfully.</h3></font></div>
					<?php } ?>
				...
				<div class="form-group">
				  <label for="comment">Views:<span class="star">*</span></label>
				  <span class="pull-left">
				  <textarea rows="5" cols="25" class="form-control" name="txtcomment" id="comment" placeholder='Views' style="width: 299px;" onkeyup="countChar(this)"><?php if(stripslashes(content_desc(htmlspecialchars($txtcomment!="")))){ echo content_desc(stripcslashes($txtcomment));} ?></textarea>
